...
一年前のこと、Subversionを長く使ってきた開発チームにGitを教えようとした。わたしはGitの*ブランチ*をちゃんと説明したかった。というのもSubversionにもブランチという用語があるが、GitのブランチはSubversionとは似て非なるものだからだ。"Git"と"Subversion"と"ブランチ"をキーにネットを検索すると記事がたくさんみつかる。だがわたしが「なるほど」と納得できる記事が見当たらなかった。

いま自分の手元にあるプロジェクトの`.git`ディレクトリ位のなかにあるGitレポジトリの実物を読み出して図にしてくれる、そういうツールがほしい。実物を図示してくれるツールがあれば`git branch xxx`とか`git commit`とか`git merge xxx`とかのコマンドを実行する段階を追いかけつつ、Gitレポジトリの状態の遷移を情報量豊かに描写し、説明できるのになあ。
...
4. `HEAD`の意味はこうだ。「*あなたが次にgit commitコマンドを実行したら新しいcommitオブジェクトができるはずだが、いまHEADが指しているcommitオブジェクトが新しいcommitオブジェクトの親(parent)になるよ*」
...
`master`ブランチに戻ろう。ファイル `README.md` を修正してコミットしよう。
...
- `echo "#Read me carefully" > README.md`やってファイルを更新した
...
- 参照名 `develop` が新しいcommitオブジェクトを指すように更新された。


### ステップ6

`develop`ブランチを`master`ブランチにマージしよう。すなわち`develop`ブランチで追加・変更・削除したファイルを`master`ブランチにとり込もう。
...
1. 増えた2個のcommitオブジェクトうちひとつは`develop`ブランチで作られたものだ。`add doc/TODO.txt`というコミットメッセージをもっている。`develop`ブランチだけで見えていたcommitオブジェクトが`git merge develop`コマンドによって`master`ブランチでも見えるようになった。

1. もうひとつ増えたcommitオブジェクトがある。このcommitオブジェクトは親(parent)コミットを2つ持っている点が特徴的だ。れをマージコミットともいう。

1. 参照名 `master` はマージコミットを指すように自動的に更新される。

1. マージコミットは普通のcommitオブジェクトと同様にルートディレクトリ `/` に対応するtreeオブジェクトへの参照をもっている。そのtreeオブジェクトを起点としてすべてのファイルのblobオブジェクトを参照することができる。それらblobはこれまでに追加・変更・削除されたファイルの最新状態が反映されている。

1. 上の図ではマージコミットが指しているtreeオブジェクトだけを描き、時系列的に古いcommitオブジェクトが指しているtreeオブジェクトを省略した。これは図を簡素化するためだ。その他のtreeオブジェクトがGitレポジトリから消えて無くなったわけではない。
...
一年前の同僚諸君に本稿を読んでほしいなあ。これならGitの*ブランチ*と*マージ*が理解できるだろうと思うのだが、どうだろうか？
