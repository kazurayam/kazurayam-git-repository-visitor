...
def visualize(wt: str):
    ...
    visualize_tree_or_blob(wt, tree_hash7)


def visualize_tree_or_blob(wt, tree_hash):
    o = GIT.lstree(wt, tree_hash)
    for line in o.splitlines():
        (mode, object_type, object_hash, file_name) = tuple(line.split())
        if object_type == "tree":
            sub_tree_hash7 = object_hash[0:7]
            visualize_tree_or_blob(wt, sub_tree_hash7)  # trace the tree recursively
